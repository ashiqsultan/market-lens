...
async function getFinancial(symbol: string): Promise<AppRes> {
  try {
    const polygonRes = await fetch(
      `https://api.polygon.io/vX/reference/financials?ticker=${symbol}&timeframe=annual&limit=3&apiKey=${POLYGON_API_KEY}`
    );
